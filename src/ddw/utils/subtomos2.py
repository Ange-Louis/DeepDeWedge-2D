import math
from src.ddw.utils.mrctools2 import load_data
import numpy as np
import torch

# ========================================================================================
# Ici , j'ai un problème puisque j'extrais des images 2D mais que je reçois des images 3d,
# je vais avoir un bug. Il faudrait que je puisse faire des feuillets. J'ai peut être un 
# problème avec les formats RGB aussi puisque la shape du tensor varie donc en fonction du
# format de l'image
# ========================================================================================
...
def extract_subtomos(
    ...
    """
    Extracts sub-tomograms of size 'subtomo_size' using a 2D sliding window approach. The two strides of the sliding window are specified by 'subtomo_extraction_strides', which must be two integers.
...
def get_linear_ramp_weights(subtomo_size, subtomo_overlap):
    """
    Produces a cubic 2D tensor containing linear weights used to average overlapping sub-tomogram parts in 'reassemble_subtomos'.
    """
    ...
    for i in range(subtomo_size):
        for j in range(subtomo_size):
            weight_map_2d[i, j] = (
                weight_map_1d[i] * weight_map_1d[j]
            )
