...
from .utils.mrctools2 import load_data
from .utils.subtomos2 import extract_subtomos
...
def prepare_data(
    ...
    min_nonzero_mask_fraction_in_subtomo: Annotated[
        Optional[float],
        typer.Option(
            help="Minimum fraction of pixels in a subtomogram that correspond to nonzero pixels in the mask. If mask_files are provided, this parameter has to be provided as well. If no mask_files are provided, this parameter is ignored."
        ),
    ] = 0.3,
    ...
    for k, (tomo0_file, tomo1_file, mask_file) in enumerate(
        ...
        tomo0 = load_data(tomo0_file).float()
        ...
        tomo1 = load_data(tomo1_file).float()
        if standardize_full_tomos:
            tomo1 -= tomo1.mean()
            tomo1 /= tomo1.std()
