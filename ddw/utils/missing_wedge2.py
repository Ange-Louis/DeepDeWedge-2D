...
from .fourier2 import get_2d_fft_freqs_on_grid
from .rotation2 import rotate_area


def get_missing_wedge_mask(grid_size, mw_angle, device="cpu"):
    """
    Produces a 2D binary mask with shape 'grid_size', which can be used to zero-out Fourier components that lie inside a missing wedge with width 'mw_angle'.
    """
    ...
    # embed normal vectors into x-y plane
    normal_left = torch.tensor([normal_left[0], normal_left[1]], device=device)
    normal_right = torch.tensor([normal_right[0], normal_right[1]], device=device)
